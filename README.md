...
| ⛶ 全螢幕 | 畫布角落與側欄各一顆鈕,整個棋盤面板放大到全螢幕;全螢幕裡留「提示/後悔/視角/離開」小工具列,工具列最左邊一顆 ▲ 可以收起來(只留這顆,棋盤更乾淨)。原生 Fullscreen API,iPhone Safari 沒有就退成 CSS 假全螢幕(同一套版面)。手機轉橫向(矮螢幕)會自動套用同一套滿版版面,不用再按一次 ⛶ |
